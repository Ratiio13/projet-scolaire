...
double Descente_vois_1(int nb_clients, int nb_usines, vector<double> couts_ouverture, vector<vector<double>> couts_affectation){
    ...
    while(limite_iteration<1000 && indice != -1){
        cout << "iteration n° " << limite_iteration << endl;
        double curent = 0;
...
vector<bool> Descente_vois_1_sol(int nb_clients, int nb_usines, vector<double> couts_ouverture, vector<vector<double>> couts_affectation,vector<bool> ouverte){
    ...
    while(limite_iteration<1000 && indice != -1){
        ...
        cout << " valeur pour l'instant " << curent ;
        limite_iteration +=1;
    }
    //cout << "indice final = "<<indice<<"\n" << " et nb iteration = "<<limite_iteration<<"\n";
    //double total = evaluate(nb_clients, nb_usines, couts_ouverture, couts_affectation, ouverte);
...
double genetique_1(int nb_clients, int nb_usines, vector<double> couts_ouverture, vector<vector<double>> couts_affectation,int nb_gen,int pop){
    int nb_population  = pop;
    vector<vector<bool>> population(nb_population, vector<bool>(nb_usines));
    // On genere la premiere generation aleatoirement
    ...
    for(int i = 0;i<nb_gen;i++ ){
        ...
        for (int j = 0; j < nb_population-1; j+=2){
            ...
            //on met la moitié d'un parent dans un enfant
            ...
            //enfants[j] = Descente_vois_1_sol(nb_clients, nb_usines, couts_ouverture, couts_affectation,population[j]);        // et on met les enfants dans le 
            //enfants[j+1] = Descente_vois_1_sol(nb_clients, nb_usines, couts_ouverture, couts_affectation,population[j+1]);   
        ...
        cout << " valeur pour l'instant " << conclave_final[0].second << endl ;
...
double Cooking_incoming(int nb_clients, int nb_usines, vector<double> couts_ouverture, vector<vector<double>> couts_affectation){
    int nb_billes = 1;//35;
    double proba = 0.95;
    // on crée les billes
    vector<vector<bool>> nuage(nb_billes, vector<bool>(nb_usines));
    ...
    cout << "on a crée le vecteur" << endl;
    /*
    for (int j = 0; j < nb_usines; j++){
        nuage[nb_billes-1][j] = 0;
    }
...
int main()  {
    ...
    std::vector<std::string> file = {"new4"};//{"capa","capb","capc"};//{"instance"} ;{"cap71", "cap72", "cap73","cap74","cap101","cap102","cap103","cap104","cap131","cap132","cap133","cap134"};
    std::vector<double> optimale = {17156454.478,12979071.582,11505594.329};//{932615.750, 977799.400,1010641.450,1034976.975,796648.437,854704.200,893782.112,928941.750,793439.562,851495.325,893076.712,928941.750};
    int nb_simulation = 1;//12;
    double resultat = 0;
    double resultat_val = 0;
    double temps = 0;
    int nb_test = 1;
    int pop = 22;
    int nb_gen = 5 ;
    //std::vector<double> liste_norm_ouv = {7350, 12250, 17150, 24500 }; 
    //std::vector<double> liste_norm_af = {51501.1, 51501.1, 51501.1, 51501.1 }; 
    //std::vector<double> liste = {51501.1, 51501.1, 51501.1, 51501.1 }; 
    //std::cout << std::fixed << std::setprecision(10);
    ...
    for(int simulation = 0; simulation < nb_simulation;simulation++){
        ...
        /*
        for(int ratio_ouv = 0; test < nb_test;test++){
            for(int ratio_ouv = 0; test < nb_test;test++){
                ouverture[i] = ouverture[i]*(1.1);
            }
            */
            for(int test = 0; test < nb_test;test++){
            //calcul de la solution
            cout << "début résol" << endl;
                auto start = std::chrono::high_resolution_clock::now();
                // pour changer d'heuristique, il faut changer cette ligne
                double v1 = genetique_1(nb_clients, nb_usines, ouverture, affectations,10,100);
                ...
                //cout << "pour "+file[simulation]+" : " << (v1/optimale[simulation]-1)*100 << " pour un temps de : " << duration.count() << " ms"<<"\n";  
                cout << "pour "+file[simulation]+" : " << v1 << " pour un temps de : " << duration.count() << " ms"<<"\n";  
    ...
    cout << "temps en moyenne : " << temps/(nb_test*nb_simulation*1000) << " s" << "\n";
    cout << temps/(nb_test*nb_simulation*1000) << " & " << resultat_val/(nb_test*nb_simulation) << " & " <<  resultat/(nb_test*nb_simulation) << "\\\\ \\hline"<<endl;
...
// il faut comparer les vitesses de convergence et les mettre dans un tab Latex
/*
ouverture ok
fermeture ok
différente visite ( premier up)
pluie de bille ok
genetique ok 

Il faut ensuite, faire les diff version de génétique
faire évoluer les enfants, avant le conclave

resultat en moyenne : 0.005
temps en moyenne : 243.176 ms



Pour afficher les moyennes
                        double somme = std::accumulate(ouverture.begin(), ouverture.end(), 0.0);
            double moyenne = somme / ouverture.size();
            cout << "pour "+file[simulation]+" moyenne C_ouverture : "<< moyenne << " ";
            
            double somme_totale = 0.0;
            std::size_t compteur = 0;

            for (const auto& ligne : affectations) {
                somme_totale += std::accumulate(ligne.begin(), ligne.end(), 0.0);
                compteur += ligne.size(); 
            }
            ...
            cout << "moyenne C_affectations : " << moyenne_af <<"\n"; 


                        auto [min_it, max_it] = std::minmax_element(ouverture.begin(), ouverture.end());
            cout << "ouverture[1] = "<< ouverture[1] <<endl; 
            cout << "écart = "<< *max_it - *min_it << endl;
            double min_val = std::numeric_limits<double>::infinity();
            double max_val = -std::numeric_limits<double>::infinity();

            for (const auto& ligne : affectations) {
                if (ligne.empty()) continue;
                auto [min_it, max_it] = std::minmax_element(ligne.begin(), ligne.end());
                if (*min_it < min_val) min_val = *min_it;
                if (*max_it > max_val) max_val = *max_it;
            }
